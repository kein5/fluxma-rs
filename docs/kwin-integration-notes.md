...
- install 経路は `install_stub()` として先に固定し、placeholder-only の間は deferred reason と候補名を返す
- install stub は frame/present に分離し、combined install summary で両方を 1 行にまとめられる
- install stub report には `source_file` と `symbol` も含め、candidate 名だけでなく具体的な KWin entry point まで返す
...
- combined preflight/install report 自体にも frame/present 単位の helper を持たせ、combined report を unpack せずに deferred reason や deferred state を追えるようにした
...
- observation report には deferred reason と preflight blocker flag の passthrough helper も持たせ、preflight/install の nested report を unpack しなくても frame/present ごとの gate 状態を追えるようにした
- plugin root の observation / install observation helper は synthetic report を使う smoke test でも固定し、frame/present の split や placeholder false 側も contract として残した
